...
This document describes the intended system and the implemented Phase 2–4
slices. Local evidence intake, structural tool isolation, SQLite results,
deterministic JSON/HTML reports, and a locally mocked first image-worker adapter
exist. The real CUDA path is code-only and has not been executed. Components
labeled “later” are not implemented.
...
flowchart TB
    ...
    subgraph Local[Local / API control plane]
        ...
        Orchestrator[Asynchronous detector orchestrator — later Phase 6]
    ...
    subgraph GPU[Detector execution plane]
        Queue[Queue-based endpoint]
        Adapter[Community Forensics worker adapter — Phase 4]
        Mock[Deterministic CPU mock — Phase 4]
        Workers[Real digest-pinned CUDA execution — later Phase 5]
    end
    ...
    Results --> Builder
    Builder --> Reports
    API -. not connected in Phase 4 .-> Orchestrator
    Orchestrator -. later .-> Queue
    Queue --> Adapter
    Adapter --> Mock
    Adapter -. future verified checkpoint .-> Workers
    Workers --> Derivatives
...
The Phase 4 adapter consumes shared contracts but is deliberately disconnected
from the API and reports. Its Mac/CI path uses generated bytes and an injected
mock. The future real path uses the same service with HTTPS retrieval, exact
input hash verification, pinned preprocessing, and a checkpoint-verifying CUDA
backend. See [Community Forensics image worker](image-detector-worker.md).
