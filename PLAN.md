...
**Only Phase 0 through Phase 3 are authorized in the current task.** Phase 3 is
complete on its feature branch. All later phases are planning context; do not
implement Phase 4 or beyond without explicit user authorization.
...
### Phase 4 — First image-detector worker adapter (not authorized)
...
Deliverables: pinned Community Forensics source/model metadata, exact official
preprocessing, input-hash verification, mocked local tests, and a Linux AMD64
CUDA Dockerfile. Weight download and a real GPU run remain explicit opt-in
operator actions; no GPU is rented or deployed in this milestone.
...
expectations are immutable and license-reviewed.
